...
changelog:
  - "0.3.0 — 2026-09-11 — the RAG/on-box-AI removal of 2026-09-01 is now drawn, not just described: the component map replaces the Qdrant/rag-api node with the markdown knowledge base, drops the on-box AI surface and adds the analysis MCP server as the external agent interface; the offline-boundary and packaging paragraphs stop describing the removed stack."
...
and an external agentic harness (Pi, Claude Code, …) can reason over an analysis those two produced
through the **analysis MCP server**. The whole thing runs **offline**; the only outbound traffic is
opt-in public-indicator lookups.
...
flowchart TB
    ...
    knowledge["Knowledge base (method/)<br/>ATT&amp;CK notes · regulations · ACN<br/>markdown, read directly"]

    reports["Reports<br/>(analysis/engine/report_*: HTML/JSON/MD)"]

    subgraph surfaces["Surfaces — GUI and CLI are each complete"]
        gui["GUI<br/>FastAPI 127.0.0.1:8700"]
        cli["CLI<br/>analysis/engine/run_*.py"]
    end

    mcp["Analysis MCP server<br/>(analyze / analyze_case / eid_lookup)<br/>external agent interface, §9-redacted"]
    ...
    knowledge -. grounds/explains .-> brain
...
6. **Report & consume.** Results become self-contained **reports** (HTML/JSON/Markdown), portable
   **bundles** (`engine/bundle.py`, evidence deliberately excluded, §9) and persistent **cases** that
   ...
   the whole job — and can be handed to an external agent through the **analysis MCP server**
   (`analysis/analysis_mcp_server.py`), which exposes the pipeline (`analyze`, `analyze_case`,
   `eid_lookup`) and returns pseudonymized findings (§9). The reasoning lives in the agent, the
   pipeline stays deterministic, and the server is deliberately **optional**: the product is whole
   without it.
...
VirusTotal, ThreatFox), egress-gated and never sent private IPs/internal domains (§9/§15).

**External AI assistants** (Pi, Claude Code, …) sit **outside** this boundary: they are
development tools used to build EventHound and, through the analysis MCP server, to reason over an
analysis — never part of the running product, and never given a real identifier (§9 pseudonymization).
...
| Knowledge base | `method/` + `analysis/analytics/attack_map.json` | markdown grounding analysis (ATT&CK, GDPR/NIS2/DORA, ACN) |
...
| Analysis MCP | `analysis/analysis_mcp_server.py` | the agent interface (`analyze` / `analyze_case` / `eid_lookup`), §9 redaction gate |
...
**repository root**, because the engine shares `tools/` and Docker cannot copy above its context; the
root `.dockerignore` is an allowlist that keeps `data/` and the local binaries out of the build
context.

On macOS the **native** stack (`./setup.sh all`) is the recommended runtime instead: containers there
are CPU-only and capped by the Docker VM's memory.
