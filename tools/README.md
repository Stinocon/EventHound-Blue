...
|---|---|---|---|
| `check.sh` | Workspace health run: leak + input guard + doc guards + static analysis (ruff) + guard smoke + analysis tests (EVTX/PCAP/logon) + golden scoring/enrichment/compliance. `--props` runs property tests. | — | Exits !=0 on any *hard* failure. |
...
| `check-doc-paths.py` | Every repository path a tracked `.md` names must exist. Markdown links, every token of a code span and of a fenced block — so a path followed by a flag, or preceded by `python3`, is checked too, and a token is only considered when it starts at a directory this repository has; prose is never parsed. `--list` shows what it checked. | hard | Documentation rots by rename: `uninstall-macos.sh` outlived the file becoming `uninstall.sh`, a download script was promised under an analysis/scripts directory that never existed, docs/crowdstrike/ survived its own deletion in four places, and the removed check-config-integrity.sh stayed an instruction in three files for four months. (The last two are named in prose rather than as code spans — the checker cannot tell an example from a reference, and it is right not to try: the first draft of this very cell named a placeholder path as a code span and turned the gate red.) Glob patterns, `<placeholders>` and frontmatter changelogs are excluded, and punctuation is trimmed off the end of a token but never off the front — a checker that guesses gets muted, which is worse than none. A missing path that **git is told to ignore** is absent by design and accepted: that used to be a hand-written list, and being incomplete made this HARD check fail in every fresh clone while staying green on the machine where the installer had run. Known limits, stated in the file: it cannot tell an artifact the reader will create from a document that was never published; a bare filename with no directory is not resolved; an unclosed fence hides its block, and an HTML comment inside one is scanned. |
...
| `check-lint.sh` | Static analysis: `ruff check` over `analysis/` and `tools/`, rule set and ignores in the root `ruff.toml`. | hard when ruff is installed, SKIP otherwise | The same contract as pytest: a fresh clone with no dev extras must stay runnable, and a check that cannot run says so instead of reporting a pass it did not earn. Its first run on the repo found 38 findings and ten were real — a word set with two members listed twice, four `raise` statements inside `except` that dropped the exception chain, unused imports and locals, a closure over a loop variable, `assert False` under `-O`. The rule set is the green subset on purpose; the value is catching the next one. |
...
| `test-guards.sh` | Smoke test of the guard scripts' BEHAVIOR in a sandbox (ephemeral fixtures), not of current repo state: `check-injection.sh` on dirty/clean files, and the reference extractor inside `check-doc-paths.py` on the shapes a rule once hid. | hard | Makes a soft guard deterministic, and pins the part of the doc gate that decides *what* gets checked — the part that had been wrong. |
